...
def fetch_soup(url: str) -> BeautifulSoup:
    ...
    """
    delay = random.uniform(*constants.REQUEST_DELAY_RANGE_SECONDS)
    ...
    for attempt in range(constants.MAX_RETRIES):
        try:
            response = requests.get(url, headers=constants.HEADERS, timeout=20)
            ...
            if status_code in [429, 503]:
                wait_time = constants.BACKOFF_FACTOR**attempt
                logger.warning(
                    f"HTTP {status_code} for {url}. Attempt {attempt + 1}/{constants.MAX_RETRIES}. "
            ...
            wait_time = constants.BACKOFF_FACTOR**attempt
            logger.warning(
                f"Network error for {url}: {e}. Attempt {attempt + 1}/{constants.MAX_RETRIES}. "
                f"Retrying in {wait_time} seconds."
            )
            time.sleep(wait_time)

    logger.critical(
        f"Failed to fetch URL after {constants.MAX_RETRIES} attempts: {url}"
    )
    raise ScrapingError(
        f"Failed to fetch {url} after {constants.MAX_RETRIES} attempts"
    ) from last_exception
